...
def generate_samples(grammar_dir, outfiles):
    ...
    f = open(os.path.join(grammar_dir, 'template.sh'))
    template = f.read()
    f.close()

    jsgrammar = Grammar()
    err = jsgrammar.parse_from_file(os.path.join(grammar_dir, 'xdotool.txt'))
    if err > 0:
...
def main():
    ...
    if multiple_samples:
        ...
        for i in range(nsamples):
            outfiles.append(os.path.join(out_dir, 'fuzz-' + str(i).zfill(5) + '.sh'))
