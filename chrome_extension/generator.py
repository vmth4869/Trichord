...
def generate_samples(grammar_dir, outfiles):
    ...
    f = open(os.path.join(grammar_dir, 'template_ext.js'))
    template = f.read()
    f.close()

    jsgrammar = Grammar()
    err = jsgrammar.parse_from_file(os.path.join(grammar_dir, 'main.txt'))
    if err > 0:
...
def main():
    ...
    if multiple_samples:
        ...
        for i in range(nsamples):
            outfiles.append(os.path.join(out_dir, 'fuzz-' + str(i).zfill(5) + '.js'))
        ...
        print("Usage:")
        print("\tpython generator.py <output file> --subsystem <subsystem_name>")
        print("\tpython generator.py --output_dir <output directory> --no_of_files <number of output files> --subsystem <subsystem_name>")
